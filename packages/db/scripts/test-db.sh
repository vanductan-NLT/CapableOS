...
[ "$(n "select count(*) from agents")" = "9" ]              && ok "9 agents seeded" || no "agents count = $(n "select count(*) from agents")"
[ "$(n "select count(*) from agents where type='human'")" = "4" ] && ok "4 humans" || no "humans"
